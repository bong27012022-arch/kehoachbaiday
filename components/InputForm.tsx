...
const MATH_STRATEGY_OPTIONS = [
    { id: 'discovery', label: 'Dạy học khám phá', icon: 'explore' },
    { id: 'project', label: 'Dạy học dự án', icon: 'assignment' },
    { id: 'stem', label: 'Dạy học STEM', icon: 'biotech' },
    { id: 'situation', label: 'Dạy học tình huống', icon: 'psychology' },
    { id: 'differentiated', label: 'Dạy học phân hóa', icon: 'tune' },
    { id: 'gamification', label: 'Gamification', icon: 'sports_esports' },
    { id: 'flipped', label: 'Lớp học đảo ngược', icon: 'swap_vert' },
    { id: 'cooperative', label: 'Học tập hợp tác', icon: 'groups' },
];

const ASSESSMENT_OPTIONS = [
    { id: 'process', label: 'Đánh giá quá trình' },
    { id: 'rubric', label: 'Rubric' },
    { id: 'portfolio', label: 'Portfolio (Hồ sơ học tập)' },
    { id: 'peer', label: 'Đánh giá đồng đẳng' },
    { id: 'self', label: 'Tự đánh giá' },
    { id: 'quiz', label: 'Kiểm tra nhanh / Quiz' },
];
...
const InputForm: React.FC<InputFormProps> = ({ onSubmit, loadingState }) => {
    ...
    const toggleMathStrategy = (label: string) => {
        setConfig(prev => {
            const current = prev.mathStrategy;
            if (current.includes(label)) {
                return { ...prev, mathStrategy: current.filter(c => c !== label) };
            } else {
                return { ...prev, mathStrategy: [...current, label] };
            }
        });
    };

    const toggleAssessment = (label: string) => {
        setConfig(prev => {
            const current = prev.assessmentMethod;
            if (current.includes(label)) {
                return { ...prev, assessmentMethod: current.filter(c => c !== label) };
            } else {
                return { ...prev, assessmentMethod: [...current, label] };
            }
        });
    };
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 flex flex-col gap-6">
                ...
                {/* Math Lesson Plan Section - Conditional */}
                {config.subject === 'Toán' && (
                    <section className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/30 dark:to-purple-900/30 rounded-xl shadow-sm border border-indigo-200 dark:border-indigo-700 p-6 relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-indigo-200/30 to-purple-200/30 rounded-bl-full"></div>
                        <div className="relative">
                            <div className="mb-6">
                                <div className="flex items-center gap-3 mb-1">
                                    ...
                                    <div>
                                        <h2 className="text-xl font-bold tracking-tight text-indigo-900 dark:text-white">Ý Tưởng Kế Hoạch Dạy Học Toán</h2>
                                        <p className="text-sm text-indigo-600 dark:text-indigo-300">Đổi mới cách tổ chức dạy học môn Toán đỉnh cao</p>
                                    </div>
                                </div>
                                <div className="h-1 w-10 bg-gradient-to-r from-indigo-500 to-purple-500 mt-2 rounded-full"></div>
                            </div>

                            {/* Math Strategies */}
                            <div className="mb-6">
                                <label className="block text-sm font-bold text-indigo-900 dark:text-white mb-3">
                                    <span className="flex items-center gap-2">
                                        <span className="material-symbols-outlined text-[18px] text-indigo-500">strategy</span>
                                        Chiến lược dạy học đổi mới
                                    </span>
                                </label>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    {MATH_STRATEGY_OPTIONS.map((option) => {
                                        const isSelected = config.mathStrategy.includes(option.label);
                                        return (
                                            <button
                                                key={option.id}
                                                onClick={() => toggleMathStrategy(option.label)}
                                                className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-bold transition-all border ${isSelected
                                                    ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white border-transparent shadow-md shadow-indigo-200'
                                                    : 'bg-white dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-200 border-indigo-100 dark:border-indigo-700 hover:border-indigo-300 hover:shadow-sm'
                                                    }`}
                                            >
                                                <span className={`material-symbols-outlined text-[18px] ${isSelected ? 'text-white' : 'text-indigo-400'}`}>{option.icon}</span>
                                                {option.label}
                                            </button>
                                        )
                                    })}
                            ...
                            <div className="mb-6">
                                <label className="block text-sm font-bold text-indigo-900 dark:text-white mb-3">
                                    <span className="flex items-center gap-2">
                                        <span className="material-symbols-outlined text-[18px] text-purple-500">fact_check</span>
                                        Phương pháp đánh giá
                                    </span>
                                </label>
                                <div className="flex flex-wrap gap-3">
                                    {ASSESSMENT_OPTIONS.map((option) => {
                                        const isSelected = config.assessmentMethod.includes(option.label);
                                        return (
                                            <button
                                                key={option.id}
                                                onClick={() => toggleAssessment(option.label)}
                                                className={`px-4 py-2 rounded-full text-sm font-bold transition-all border ${isSelected
                                                    ? 'bg-purple-600 text-white border-purple-600 shadow-sm'
                                                    : 'bg-white dark:bg-purple-900/20 text-purple-700 dark:text-purple-200 border-purple-100 dark:border-purple-700 hover:border-purple-300'
                                                    }`}
                                            >
                                                {option.label}
                                            </button>
                            ...
                            {/* Plan Idea */}
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-indigo-900 dark:text-indigo-100">
                                    <span className="flex items-center gap-2">
                                        <span className="material-symbols-outlined text-[18px] text-indigo-500">edit_note</span>
                                        Ý tưởng kế hoạch cụ thể
                                    </span>
                                </label>
                                <textarea
                                    className="w-full h-28 rounded-xl bg-white dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-700 px-4 py-3 text-indigo-900 dark:text-white placeholder-indigo-300 dark:placeholder-indigo-500 focus:ring-2 focus:ring-indigo-400/50 transition-all resize-none"
                                    placeholder="Ví dụ: Tổ chức hoạt động khám phá định lý Pytago qua thí nghiệm cắt ghép hình, sử dụng GeoGebra để minh họa trực quan, kết hợp trò chơi đối nhóm..."
                                    value={config.mathPlanIdea}
                                    onChange={(e) => setConfig({ ...config, mathPlanIdea: e.target.value })}
                                ></textarea>
                                <p className="text-xs text-indigo-500 dark:text-indigo-400">
                                    *Mô tả càng chi tiết, AI sẽ tạo kế hoạch càng cụ thể và phù hợp.
                                </p>
                            </div>
                        </div>
                    </section>
