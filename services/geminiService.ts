...
const lessonPlanSchema: Schema = {
  ...
  properties: {
    ...
    lessonPlanHtml: { type: Type.STRING },
  },
...
export const generateLessonPlan = async (input: LessonInput, apiKey: string, model: string): Promise<LessonPlanResponse> => {
  ...
  const prompt = `
    ...
    ${input.config.subject === 'Toán' ? `
    --- THÔNG TIN KẾ HOẠCH DẠY HỌC TOÁN ĐỔI MỚI ---
    - Chiến lược dạy học đổi mới: ${input.config.mathStrategy?.length ? input.config.mathStrategy.join(', ') : 'Tự đề xuất phù hợp'}
    - Phương pháp đánh giá: ${input.config.assessmentMethod?.length ? input.config.assessmentMethod.join(', ') : 'Đánh giá quá trình'}
    - Ý tưởng kế hoạch: ${input.config.mathPlanIdea || 'Tự đề xuất kế hoạch dạy học đổi mới, sáng tạo'}
    YÊU CẦU: Tạo lessonPlanHtml chứa kế hoạch bài dạy Toán chi tiết, đổi mới, chuyên nghiệp theo cấu trúc chuẩn CT 2018.
    ` : ''}
